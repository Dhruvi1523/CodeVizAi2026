...
export default function QueueVisualizer() {
  ...
  const handleEnqueue = () => {
    ...
    setNodes((prev) => [...prev, node]); // enqueue at end
    setIsBusy(true);
  ...
  const handleDequeue = () => {
    ...
    setTimeout(() => {
      setNodes((prev) => prev.slice(1)); // remove front
      setIsBusy(false);
      addHistory("dequeue", removed.value);
    }, ANIM_MS);
  };
  ...
  const handlePeek = () => {
    ...
    setMessage(`Front: ${nodes[0].value}`);
  };
  ...
  function VisualNode({ node }) {
    ...
    return (
      <div style={style} className="bg-blue-500 text-white">
        {v}
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] flex items-center justify-center p-8 text-white">
      <div className="max-w-7xl w-full">
        <h1 className="text-3xl font-extrabold mb-2 text-white">Queue</h1>
        <h4 className="text-3xl font-extrabold mb-2 text-white">
        ...
        >
          ...
          <div className="col-span-2 flex flex-col items-center">
            <div
              className="relative w-full min-h-[120px] flex flex-row items-center justify-start border border-gray-700 rounded-xl p-4"
              aria-label="Queue container"
            >
              {nodes.length === 0 ? (
                <div className="text-gray-400">Empty. Enqueue to add items.</div>
              ) : (
          ...
          <div className="bg-[#0b1220] border border-gray-800 rounded-xl p-6 shadow-lg">
            <div className="mb-4 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-300">Front:</span>
                <div className="bg-blue-600 px-3 py-1 rounded font-bold text-sm">
                  {front ?? "—"}
                </div>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-300">Rear:</span>
                <div className="bg-blue-600 px-3 py-1 rounded font-bold text-sm">
                  {rear ?? "—"}
                </div>
              </div>
                ...
                <ul className="space-y-1 text-sm">
                  {history.map((h) => (
                    <li key={h.id} className="flex items-center gap-2">
                      ...
                      <span>
                        {h.type === "enqueue" ? "Enqueue" : "Dequeue"}: {h.val}
                      </span>
                    </li>
                  ))}
                </ul>
