...
function TopologicalSortVisualization() {
  const [nodes, setNodes] = useState([0, 1, 2, 3, 4, 5]);
  const [edges, setEdges] = useState([
    [5, 2], [5, 0], [4, 0], [4, 1], [2, 3], [3, 1]
  ]);
  const [newNodeId, setNewNodeId] = useState(6);
  const [fromNode, setFromNode] = useState(5);
  const [toNode, setToNode] = useState(2);
  ...
  const runTopologicalSort = () => {
    ...
    nodes.forEach(n => { visited[n] = false; finishTime[n] = null; });
    ...
    const dfs = (u) => {
      ...
      newSteps.push({
        ...
        finishTime: { ...finishTime },
        current: u,
        description: `Node ${u} finished → f(${u}) = ${time}`
      });
    };
    ...
    setTopoOrder(finalOrder);

    newSteps.push({
      visited: { ...visited },
      finishTime: { ...finishTime },
      current: null,
      description: `Topological Sort Complete! Order: ${finalOrder.join(" → ")}`
    });
  ...
  const currentState = steps[currentStep] || {
    ...
    finishTime: Object.fromEntries(nodes.map(n => [n, null])),
    current: null,
    description: "Click 'Run Topological Sort' to begin visualization",
  };
  ...
  const drawGraph = () => {
    return (
      <svg className="w-full h-[520px] bg-gray-900 rounded-xl shadow-2xl" viewBox="0 0 600 520">
        {/* Small arrowheads - exactly like DFS/BFS */}
        <defs>
          <marker
            id="arrowhead"
            markerWidth="12"
        ...
        {edges.map(([u, v], i) => {
          ...
          const isActive = currentState.current === u;
          const isVisitedBoth = currentState.visited[u] && currentState.visited[v];

          const strokeColor = isActive ? "#f472b6" : isVisitedBoth ? "#60a5fa" : "#4b5563";
        ...
        {nodes.map(n => {
          ...
          return (
            <g key={n}>
              ...
              {currentState.finishTime[n] !== null && (
                <text
                  x={pos.x}
                  y={pos.y + 50}
                  textAnchor="middle"
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col">
      ...
      <div className="w-full bg-gradient-to-r from-purple-900 via-blue-900 to-indigo-900 shadow-2xl">
        <div className="max-w-7xl mx-auto px-6 py-8 flex items-center justify-between">
          <h1 className="text-4xl md:text-5xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-400">
  ...
  <span className="bg-gradient-to-r from-cyan-400 via-blue-400 to-pink-400 
    ...
    left most side
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 py-10">
        {/* Controls Panel - IDENTICAL to DFS */}
        <div className="bg-gray-900/80 backdrop-blur border border-gray-800 rounded-2xl shadow-2xl p-8 mb-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div>
              ...
              <div className="text-gray-400 text-sm mb-2">Directed Graph Only (for Topological Sort)</div>
            </div>
            ...
            <div>
              <h3 className="text-2xl font-bold text-yellow-400 mb-4">Current DAG</h3>
        ...
        {/* Final Order - Beautiful box */}
        {topoOrder.length > 0 && (
          <div className="text-center mb-10">
            <div className="inline-block bg-gradient-to-r from-purple-900/90 to-pink-900/90 backdrop-blur rounded-2xl px-16 py-8 border-4 border-purple-600 shadow-2xl">
              <h2 className="text-4xl font-bold text-yellow-300 mb-6">Topological Order</h2>
        ...
        {/* Main Visualization - Same grid as DFS */}
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          {/* Side Panel - Exactly like DFS */}
          <div className="space-y-6">
            <div className="bg-gray-900/80 backdrop-blur border border-gray-800 rounded-2xl p-6 shadow-2xl">
              <h3 className="text-xl font-bold text-purple-400 mb-4">DFS Recursion Stack</h3>
            ...
            <div className="bg-gray-900/80 backdrop-blur border border-gray-800 rounded-2xl p-6 shadow-2xl">
              <h3 className="text-xl font-bold text-green-400 mb-4">Finishing Times</h3>
        ...
        {/* Playback Controls - 100% identical to DFS */}
        {steps.length > 0 && (
