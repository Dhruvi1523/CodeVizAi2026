...
function BFSVisualization() {
  ...
  // Same layout as DFS: Start node on top
  ...
  const runBFS = () => {
    ...
    newSteps.push({
      ...
      parent: { ...parent },
      current: null,
      description: `Starting BFS from node ${startVertex}`
    });
    ...
    while (queue.length > 0) {
      ...
      newSteps.push({
        ...
        parent: { ...parent },
        current: u,
        description: `Dequeued node ${u} → exploring neighbors`
      });
      ...
      (adj[u] || []).forEach(v => {
        if (!visited[v]) {
          ...
          newSteps.push({
            ...
            parent: { ...parent },
            current: u,
            description: `Discovered ${v} → enqueued (level ${queue.includes(v) ? 'next' : 'current'})`
          });
        }
      });
    }
      ...
      <svg className="w-full h-[520px] bg-gray-900 rounded-xl shadow-2xl" viewBox="0 0 600 520">
        ...
        <defs>
          ...
          <marker
            ...
            markerUnits="strokeWidth"
          >
            <path d="M0,0 L0,12 L18,6 z" fill="#38bdf8" />
          </marker>
        </defs>
        ...
        {edges.map(([u, v], i) => {
          ...
          const strokeColor = isTreeEdge ? "#38bdf8" : isVisitedBoth ? "#6ee7b7" : "#4b5563";
          ...
          return (
            <g key={n}>
              <circle
                ...
                cy={pos.y}
                r="32"
                fill={isCurrent ? "#f59e0b" : isVisited ? "#34d399" : "#1f2937"}
                stroke={isCurrent ? "#ea580c" : isVisited ? "#10b981" : "#6b7280"}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          <div className="xl:col-span-2 space-y-8">
            <div className="bg-gray-900/90 backdrop-blur border border-gray-800 rounded-2xl shadow-2xl p-6">
              <h3 className="text-3xl font-bold text-center mb-6 bg-gradient-to-r from-cyan-400 to-yellow-400 bg-clip-text text-transparent">
                Graph Visualization
              </h3>
