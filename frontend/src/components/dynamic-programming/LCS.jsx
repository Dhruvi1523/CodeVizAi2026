...
export default function LCSInteractivePage() {
  ...
  function explanationLeft() {
    if (stepIdx < 0) {
      return (
        <div className="text-slate-300 text-sm">
          Press <b className="text-sky-300">Compute</b> then Play / Next / Prev
          to animate.
        </div>
      );
    }
    ...
    if (s.type === "base") {
      return (
        <div className="space-y-1">
          ...
          <div className="text-slate-300 text-sm">
            Either <b>A index = {dispI}</b> or <b>B index = {dispJ}</b> means
            empty prefix <br />→ value ={" "}
    ...
    if (s.type === "match") {
      return (
        <div className="space-y-1">
          ...
          {/* Adjusted background for contrast: from-[#0f1b30] */}
          <div className="text-sm text-slate-300 font-mono bg-[#162740] px-2 py-1 rounded"> 
    ...
    return (
      <div className="space-y-1">
        ...
        {/* Adjusted background for contrast: from-[#30181840] */}
        <div className="text-sm text-slate-300 font-mono bg-[#162740] px-2 py-1 rounded">
    ...
    if (type === "match") {
      return (
        <div className="space-y-3 text-sm">
          ...
          {/* Adjusted background for inactive rule: from-[#0b1220] */}
          <div className="p-2 bg-[#162740] text-slate-300 rounded-lg">
            max(LCS({dispI - 1}, {dispJ}), LCS({dispI}, {dispJ - 1}))
          </div>
        </div>
      );
            ...
            {A.split("").map((ch, idx) => {
              ...
              return (
                <div key={idx} className="text-center">
                  ...
                  {/* Adjusted default background for string character boxes: from-[#0b1220] */}
                  <div
                    className={`px-3 py-2 rounded-md border border-slate-700 ${flash} ${
                      !flash && "bg-[#162740] text-white"
                    }`}
                  >
                    {ch}
                  </div>
            ...
            {Array.from({ length: n + 1 }).map((_, i) => (
              <tr key={`row-${i}`}>
                ...
                {Array.from({ length: m + 1 }).map((_, j) => {
                  ...
                  // Adjusted default cell background: from-[#0b1220]
                  let bg = "bg-[#162740] text-white border border-slate-700";

                  if (onPath)
                    bg = "bg-yellow-300 text-black border border-yellow-500";
                  else if (active) {
                  ...
                  return (
                    <td
                      key={`cell-${i}-${j}`}
                      className={`px-3 py-2 text-center transition-all ${bg}`}
                    >
                      {filled ? dp[i][j] : ""}
                    </td>
                  );
  ...
  <header className="flex items-center gap-4">
    <Link
      ...
      className="p-2 text-[#94a3b8] hover:bg-[#334155] hover:text-[#f1f5f9] rounded-full transition-colors"
  ...
  {/* Controls */}
  <div className="px-4 py-5 bg-[#1e293b] rounded border border-[#334155] flex flex-wrap items-center gap-3 text-sm shadow-inner">
    <input
      value={A}
      onChange={(e) => setA(e.target.value.toUpperCase())}
      placeholder="String A"
      className="bg-[#0f172a] text-[#f1f5f9] px-3 py-2 rounded-md border border-[#334155] focus:ring-2 focus:ring-[#6366f1] focus:outline-none"
    />
    ...
    <button
      ...
      className="bg-[#6366f1] hover:bg-[#6366f1]/80 h-10 px-6 rounded-md font-bold text-black transition"
    >
      Compute
    </button>
    ...
    <button
      ...
      className="px-4 py-1 rounded-lg bg-[#0b1220] hover:bg-[#111d33] border border-[#334155] shadow"
    >
      {playing ? "Pause" : "Play"}
    </button>
    ...
    <button
      onClick={jumpToEnd}
      className="px-4 py-1 rounded-lg bg-[#0b1220] border border-[#334155] shadow hover:bg-[#111d33]"
    >
      Jump
    </button>
    ...
    <button
      ...
      className="px-4 py-1 rounded-lg bg-[#facc15] text-black font-semibold shadow hover:bg-[#facc15]/80"
    >
      LCS
    </button>

    <div className="flex items-center gap-2 ml-auto">
      <label className="text-xs text-[#94a3b8]">Speed</label>
      ...
      <input
        ...
        className="h-2 accent-[#6366f1]"
      />

      <button
        onClick={resetAll}
        className="px-3 py-1 rounded-lg border border-[#334155] hover:bg-[#111d33] shadow"
      >
        Reset
      </button>
    </div>
  ...
  <div className="flex flex-grow overflow-hidden gap-2 px-2 py-2">
    {/* String Animation */}
    <div className="w-[40%] min-w-[260px] p-4 bg-[#1e293b] rounded-lg border border-[#334155] shadow-xl flex flex-col overflow-hidden justify-center">
      {renderStringsLeft()}
    </div>

    {/* DP Table */}
    <div className="flex-grow bg-[#1e293b] rounded-lg border border-[#334155] p-4 shadow-xl overflow-hidden flex items-center justify-center">
      <div className="overflow-auto max-h-full max-w-full border border-[#334155] rounded-lg shadow-md">
        {renderDPTable()}
      </div>
    </div>
  </div>
  ...
  {/* Explanation + Rules */}
  <div className="h-[175px] bg-[#1e293b] rounded border-t border-[#334155] grid grid-cols-3 gap-4 p-3 shadow-inner">
    {/* Explanation */}
    <div className="col-span-2 bg-[#111d33] border border-[#334155] rounded-xl shadow-lg p-3 flex flex-col">
      <div className="text-lg font-bold text-[#6366f1] mb-1">Explanation</div>
      <div className="text-sm leading-relaxed overflow-y-auto flex-1 pr-2 text-[#94a3b8]">
        {explanationLeft()}
      </div>
    </div>

    {/* Rules */}
    <div className="bg-[#111d33] border border-[#334155] rounded-xl shadow-lg p-3 flex flex-col">
      <div className="text-lg font-bold text-[#6366f1] mb-1">Rules</div>
      <div className="text-sm overflow-y-auto flex-1 text-[#94a3b8]">
        {rulesPanel()}
      </div>
    </div>
  </div>
