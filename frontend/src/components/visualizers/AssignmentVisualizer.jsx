...
// Note: scale and updateScale are passed down from TraceLayout.
const AssignmentVisualizer = ({ event, scale, updateScale }) => {
    
    // Ref to identify the content wrapper for measurement
    ...
    // Determine which sub-visualizer to render based on event type
    let SubVisualizer;
    if (event.type === "binary_operation") {
        SubVisualizer = BinaryOperationVisual;
    } else if (event.type === "subscript_assign") {
        SubVisualizer = SubscriptAssignVisual;
    } else {
        SubVisualizer = SimpleAssignmentVisual;
    }

    return (
        // Outer wrapper applies the calculated scale transformation
        <div 
            ref={contentRef} 
            className="visualizer-content-wrapper inline-block h-full w-full" // Use inline-block for scrollWidth to work
            style={{
        ...
        >
            <div className="relative z-10 h-full w-full flex items-center justify-center">
                 {/* Render the selected sub-visualizer, passing props through */}
                <SubVisualizer event={event} scale={scale} updateScale={updateScale} /> 
            </div>
        </div>
    );
};
...
// --- Sub-Visualizers must now accept scale (though they won't use it directly) ---

const SimpleAssignmentVisual = ({ event, scale, updateScale }) => { // ACCEPTED SCALE PROPS
...
    return (
        // Note: Removed relative z-10, h-full w-full from here as the wrapper already contains it
        <div className="flex flex-col items-center justify-center space-y-4 p-4 font-mono sm:space-y-6">
...
const BinaryOperationVisual = ({ event, scale, updateScale }) => { // ACCEPTED SCALE PROPS
  const { target_var, left_str, right_str, op_str, left_val, right_val, result_val } = event;
  return (
    // Note: Removed relative z-10, h-full w-full from here
    <div className="flex flex-col items-center justify-center space-y-4 p-4 font-mono sm:space-y-8">
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-base text-cyan-300 sm:text-lg">
...
const SubscriptAssignVisual = ({ event, scale, updateScale }) => { // ACCEPTED SCALE PROPS
...
// --- Helper Components (Unchanged as they don't need scale props) ---
